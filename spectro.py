...
def spectrogram_from_image(
    ...
    try:
        data = data[::-1, :, 0]  #explain this code?
    except IndexError:
        data = data[::-1, :]
    # Invert


    # explain the function of the code above:
...
image = eg.fileopenbox('Select an image file')
image = Image.open(image)

filenameWav = 'spec2wav_'+str(time.time())+'.wav'
filenameMp3 = 'spec2wav_'+str(time.time())+'.mp3'
filedir = os.path.join('logs','music','spec2wav')
...
try:
    ...
    wavfile.write(os.path.join(filedir,filenameWav), 4400, data)
...
try:
    mp3 = mp3_bytes_from_wav_bytes(wav_bytes)
    with open(os.path.join(filedir, filenameMp3), 'wb') as f:
        f.write(mp3.read())
except:
    traceback.print_exc()
